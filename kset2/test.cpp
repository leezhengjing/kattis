...
struct BSTVertex {
  // all these attributes remain public to slightly simplify the code although
    ...
    if (T->key == v) {                                // the node to be deleted
      if (T->left == NULL && T->right == NULL)        // this is a leaf
        ...
        T = T->right;                 // bypass T
                 ...
                 T->right == NULL) { // only one child at left
...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    printf("BST, height: %d, inorder traversal:",
           T->getHeight()); // equal or taller than A->getHeight()
