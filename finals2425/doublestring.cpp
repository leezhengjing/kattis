...
void solve() {
  ...
  // 1. Build Suffix Array for LCP (Longest Common Prefix)
  CPString::SuffixArray SA_fwd(s);

  // 2. Build Suffix Array on reversed string for LCS (Longest Common Suffix)
  string rev_s = s;
  reverse(rev_s.begin(), rev_s.end());
  CPString::SuffixArray SA_rev(rev_s);

  ll double_strings_count = 0;

  // 3. The Checkpoint Algorithm in pure O(N log N)
  for (int len = 1; len <= n / 2; ++len) {
    // Step by 'len' to hit our checkpoints
    for (int i = 0; i + len < n; i += len) {
      int u = i;
      int v = i + len;

      // O(1) Prefix Match from the original string
      int lcp = SA_fwd.get_lcp(u, v);

      // O(1) Suffix Match from the reversed string
      int lcs = SA_rev.get_lcp(n - 1 - u, n - 1 - v);

      // Bound them by 'len' to prevent double-counting
      // squares that belong to the next checkpoint pair
      lcp = min(lcp, len);
      lcs = min(lcs, len);

      if (lcp + lcs > len) {
        double_strings_count += (lcp + lcs - len);
      }
    }
  }

  cout << double_strings_count << "\n";
}
