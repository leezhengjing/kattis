...
void solve() {
  ...
  if (isValidUnary(X) && isValidUnary(Y) && isValidUnary(Z)) {
    ll Xval = X.length();
    ll Yval = Y.length();
    ll Zval = Z.length();
    ...
    if (valid_div && Xval <= MAX_VAL && Yval <= MAX_VAL && Zval <= MAX_VAL) {
      if (binop(Xval, Yval, op) == Zval) {
        res += "1";
      }
    }
  }
  ...
  int max_digit = 0;
  string all_chars = X + Y + Z;
  for (char c : all_chars) {
    max_digit = max(max_digit, charVal(c));
  }

  int min_base = max(2, max_digit + 1);

  for (int i = min_base; i <= 36; ++i) {
    ll Xval = CPMath::toBase10(X, i);
    ll Yval = CPMath::toBase10(Y, i);
    ll Zval = CPMath::toBase10(Z, i);

    if (op == '/' && Xval % Yval != 0)
      continue;
    if (binop(Xval, Yval, op) == Zval) {
      res += (i < 10 ? (char)(i + '0')
                     : (i == 36 ? (char)'0' : (char)(i - 10 + 'a')));
    }
  }
  if (res.empty())
    cout << "invalid\n";
