...
const int pts[] = {0, 0, 0, 1, 1, 2, 3, 5, 11};

const int dr[] = {-1, -1, -1, 0, 0, 1, 1, 1};
const int dc[] = {-1, 0, 1, -1, 1, -1, 0, 1};

CPString::FastTrie<2500000, 'A'> trie;
vector<string> board(4);
set<string> found_words;

void dfs(int r, int c, int u, int mask, string curr_word) {
  int idx = board[r][c] - 'A';

  u = trie.next_node[u][idx];
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    if (nr >= 0 && nr < 4 && nc >= 0 && nc < 4) {
      ...
      if (!(mask & bit)) {
        dfs(nr, nc, u, mask | bit, curr_word);
      }
    }
  }
}
void solve() {
  ...
  while (b--) {
    ...
    for (int r = 0; r < 4; ++r) {
      for (int c = 0; c < 4; ++c) {
        int initial_mask = 1 << (r * 4 + c);

        dfs(r, c, 0, initial_mask, "");
      }
    }
    ...
    cout << total_score << " " << longest_word << " " << found_words.size()
         << "\n";
  }
}
