...
void solve() {
  ...
  int n;
  cin >> n;
  unordered_map<int, char> map = {{0, '/'}, {1, '*'}, {2, '-'}, {3, '+'}};
        ...
        while (!operators.empty()) {
          ...
          if (!operators.empty()) {
            if (!check_precedence(next, operators.front())) {
              ...
              operands.pop_front();
              int third_op = operands.front();
              operands.pop_front();
              operands.emplace_front(operate(second_op, third_op, next));
              operands.emplace_front(first_op);
              continue;
            }
          }
          int first_op = operands.front();
