...
void solve() {
  ...
  vector<bool> uncategorised(m + 1, true);

  auto get_toy_idx = [&](int toy_idx) { return toy_idx + p; };
  auto get_child_idx = [&](int child_idx) { return child_idx + m + p; };

  for (int child_idx = 1; child_idx <= n; ++child_idx) {
    int k;
    cin >> k;
    ...
    for (int j = 0; j < k; ++j) {
      ...
      mf.add_edge(get_toy_idx(toy_idx), get_child_idx(child_idx), 1);
    }

    mf.add_edge(get_child_idx(child_idx), T, 1);
  }

  for (int category_idx = 1; category_idx <= p; ++category_idx) {
    int l;
    cin >> l;
    ...
    for (int j = 0; j < l; ++j) {
      int toy_idx;
      cin >> toy_idx;
      mf.add_edge(category_idx, get_toy_idx(toy_idx), 1);
      uncategorised[toy_idx] = false;
    }

    int r;
    cin >> r;

    mf.add_edge(S, category_idx, r);
  }

  for (int i = 1; i <= m; ++i) {
    if (uncategorised[i]) {

      mf.add_edge(S, get_toy_idx(i), 1);
    }
  }
