...
void solve() {
  ...
  FastMaxFlow mf(T + 1);
  mf.reserve_edges(S, m);

  for (int i = 1; i <= m; ++i) {
    mf.reserve_edges(i, n + 1);
  }

  for (int i = 1; i <= n; ++i) {
    mf.reserve_edges(i + m, m + 1);
  }

  mf.reserve_edges(T, n);

  vi rows(m + 1), cols(n + 1);
  for (int i = 1; i <= m; ++i) {
    cin >> rows[i];
    mf.add_edge(S, i, rows[i]);
  }

  for (int i = 1; i <= n; ++i) {
    cin >> cols[i];
    mf.add_edge(i + m, T, cols[i]);
  }
  ...
  int res_sum = accumulate(rows.begin(), rows.end(), 0);
  int cols_sum = accumulate(cols.begin(), cols.end(), 0);
