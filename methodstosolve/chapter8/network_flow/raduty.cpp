...
public:
  ...
  void print_schedule(int m, int n, const vector<string> &ra_names) {
    for (int d = 1; d <= n; ++d) {
      int day_node = m + d;
      cout << "Day " << d << ":";
...
void solve() {
  ...
  MaxFlow final_mf(T + 1);
  for (int i = 1; i <= m; ++i)
    final_mf.add_edge(S, i, res);
  for (int u = 1; u <= m; ++u) {
    for (int v : avail[u]) {
      final_mf.add_edge(u, v + m, 1);
    }
  }
  for (int i = 1; i <= n; ++i) {
    final_mf.add_edge(i + m, T, 2);
  }

  final_mf.dinic(S, T);

  cout << res << "\n";
  final_mf.print_schedule(m, n, ra_names);
}
