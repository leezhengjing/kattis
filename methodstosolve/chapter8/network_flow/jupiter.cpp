...
void solve() {
  ...
  auto in = [&](int w, int k) { return (w - 1) * (2 * q + 1) + k; };
  auto out = [&](int w, int k) { return (w - 1) * (2 * q + 1) + q + k; };
  auto win = [&](int w) { return w * (2 * q + 1); };

  int S = 0;
  int T = (2 * q + 1) * n + 1;

  MaxFlow mf(T + 1);
  // i is window i, 1 indexed
  ll total_data_generated = 0;
  for (int w = 1; w <= n; ++w) {
    int window_limit;
    cin >> window_limit;
    // window to sink T edge;
    mf.add_edge(win(w), T, window_limit);
    vll data_for_queue(q + 1, 0);
    for (int j = 0; j < s; ++j) {
      // j is sensor j, 0th index, convert sensor to queue, then draw edge from
      // source to queue_in and queue_out
      ll cap;
      cin >> cap;
      data_for_queue[sensor_to_queue[j]] += cap;
      total_data_generated += cap;
    };

    for (int k = 1; k <= q; ++k) {
      if (data_for_queue[k] > 0)
        mf.add_edge(S, in(w, k), data_for_queue[k]);

      mf.add_edge(in(w, k), out(w, k), queue_limits[k]);
      mf.add_edge(out(w, k), win(w), INF);
      if (w < n)
        mf.add_edge(out(w, k), in(w + 1, k), queue_limits[k]);
    }
  }
  ll res = mf.dinic(S, T);
  if (res == total_data_generated)
    cout << "possible\n";
  else
    cout << "impossible\n";
}
