...
void solve() {
  ...
  for (int u = 1; u <= n; ++u) {
    for (int time = 0; time < s; ++time) {
      mf.add_edge(u + time * n, u + (time + 1) * n, INF);
    }
  }
  for (int time = 0; time <= s; ++time) {
    for (auto road : roads) {
      if (time + road.t <= s)
        mf.add_edge(road.a + time * n, road.b + (time + road.t) * n, road.p,
                    true);
    }
  }

  for (auto medic_id : medic_ids) {
    for (int time = 0; time <= s; ++time) {
      mf.add_edge(medic_id + time * n, T, INF);
    }
  }
