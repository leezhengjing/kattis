...
void solve() {
  int B, G;
  cin >> B >> G;
  // vector<Person> boys(B);
  // vector<Person> girls(G);
  map<string, int> book_to_boy;
  for (int i = 0; i < B; ++i) {
    string name;
    cin >> name;
    int k;
  ...
  //     string bookname;
  //     cin >> bookname;
  //     girls[i].books.push_back(bookname);
  //   }
  // }
  ...
  for (int i = 0; i < G; ++i) {
    ...
    for (int j = 0; j < k; ++j) {
      ...
      if (book_to_boy.count(bookname)) {
        int boy_idx = book_to_boy[bookname];
        mf.add_edge(boy_idx, i + B, 1);
      }
    }
  }
