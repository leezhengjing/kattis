...
void solve() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (gender == 'M') {
      men.push_back({height, music, sport});
    } else {
      women.push_back({height, music, sport});
    }
  }
  ...
  for (int u = 0; u < M; ++u) {
    mf.add_edge(S, u, 1);
    for (int v = 0; v < W; ++v) {
      if (abs(men[u].height - women[v].height) <= 40 &&
          men[u].music == women[v].music && men[u].sport != women[v].sport) {
        mf.add_edge(u, v + M, 1);
      }
    }
  }

  int max_matchings = mf.dinic(S, T);

  cout << N - max_matchings << "\n";
}
