...
void solve() {
  ...
  vvi horizontal_segments(Y + 1, vi(X + 1, 0));
  int h_seg_idx = 0;
  for (int i = 1; i <= Y; ++i) {
    h_seg_idx++;
    for (int j = 1; j <= X; ++j) {
      if (grid[i][j] == '#') {
        ...
        horizontal_segments[i][j] = h_seg_idx;
      }
    }
  }

  vvi vertical_segments(Y + 1, vi(X + 1, 0));
  int v_seg_idx = 0;
  for (int j = 1; j <= X; ++j) {
    v_seg_idx++;
    for (int i = 1; i <= Y; ++i) {
      if (grid[i][j] == '#') {
        ...
        vertical_segments[i][j] = v_seg_idx;
      }
    }
  }

  int S = 0;
  int T = h_seg_idx + v_seg_idx + 1;
  MaxFlow mf(T + 1);

  for (int i = 1; i <= h_seg_idx; ++i) {
    mf.add_edge(S, i, 1);
  }

  for (int i = 1; i <= v_seg_idx; ++i) {
    mf.add_edge(h_seg_idx + i, T, 1);
  }

  for (int i = 1; i <= Y; ++i) {
    for (int j = 1; j <= X; ++j) {
      if (grid[i][j] == '*') {
        mf.add_edge(horizontal_segments[i][j],
                    h_seg_idx + vertical_segments[i][j], 1);
      }
    }
  }
